...
pub fn calculate_crispi(mainboard: &[Card], commanders: &[Card], n_gc: u32) -> CrispiEvaluation {
    ...
    let commander_engine_signal = commanders
        .iter()
        .filter(|c| {
            let r = infer_roles(c);
            let is_engine =
    ...
    let win_package_density = if non_land_count > 0 {
        wincon_count_efficient as f32 / non_land_count as f32
    } else {
        0.0
    };
    ...
    let consistency = CrispiDimension {
        ...
        justification: format!(
            "Weighted consistency value: {:.2}. Premium tutors: {}.{}",
    ...
    let raw_score = (consistency.score
        + resilience.score
        + interaction.score
        + speed.score
        + pivotability.score) as f32;
        ...
        if card_map.contains_key(&norm_a) && card_map.contains_key(&norm_b) {
            ...
            if prereqs_met {
                ...
                detected_combos.push(format!(
                    "{} + {}{}",
                    combo.card_a, combo.card_b, effects_str
                ));
                ...
                for card in cards {
                    ...
                    if (card.is_creature() || card.is_artifact())
                        && !has_non_tapping_activation(&oracle)
                    {
    ...
    for combo in THREE_CARD_COMBOS {
        ...
        if card_map.contains_key(&norm_a)
            && card_map.contains_key(&norm_b)
            && card_map.contains_key(&norm_c)
        {
            let card_a = card_map.get(&norm_a).unwrap();
            let card_b = card_map.get(&norm_b).unwrap();
            ...
            if prereqs_met {
                ...
                detected_combos.push(format!(
                    "{} + {} + {}{}",
                    combo.card_a, combo.card_b, combo.card_c, effects_str
                ));

                let mut combo_bonus = 0.02 + tutor_influence;

                let total_mv =
                    (card_a.mana_value() + card_b.mana_value() + card_c.mana_value()) as f32;
    ...
    let nonland_mana_source_count = mainboard
        .iter()
        .filter(|card| {
            if card.is_land() {
                return false;
